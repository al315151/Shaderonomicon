Shader "Shaderonomicon/TestExport"
...
SubShader{Tags { "LightMode" = "ForwardBase" "RenderType" = "Opaque" }
LOD 100   
Pass  {CGPROGRAM  
#pragma vertex vert  
...
struct vertexOutput    {float4 pos : SV_POSITION;
...
float3 lightDirection;float attenuation;
if (0.0 == _WorldSpaceLightPos0.w)
{attenuation = 1.0f;lightDirection = normalize(_WorldSpaceLightPos0.xyz);}
else  {float3 vertexToLightSource = _WorldSpaceLightPos0.xyz - input.worldPosition.xyz;
float distance = length(vertexToLightSource);attenuation = 1.0 / distance
...
else if (LightModel == 3){float3 lightDirection;
float attenuation;if (0.0 == _WorldSpaceLightPos0.w)
{attenuation = 1.0f;lightDirection = normalize(_WorldSpaceLightPos0.xyz);}
else  {float3 vertexToLightSource = _WorldSpaceLightPos0.xyz - input.worldPosition.xyz;
float distance = length(vertexToLightSource);attenuation = 1.0 / distance;
lightDirection = normalize(vertexToLightSource);}
...
return float4(finalColor, 1.0);}else if (LightModel == 4)
{float3 viewDirection = normalize(_WorldSpaceCameraPos - input.worldPosition.xyz);
float3 lightDirection;float attenuation;
if (0.0 == _WorldSpaceLightPos0.w){attenuation = 1.0f;
